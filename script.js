...
function renderMessage(msg){const div=document.createElement('div');div.className='px-4 py-2 rounded-2xl max-w-[70%] break-words transition-all duration-300 flex items-center gap-2';const tipo=msg.from===currentUserId?'sent':'received';div.style.backgroundColor=tipo==='sent'?getComputedStyle(document.documentElement).getPropertyValue('--color-sent')||'#007AFF':getComputedStyle(document.documentElement).getPropertyValue('--color-received')||'#E5E5EA';div.style.color=tipo==='sent'?'white':'black';div.classList.add(tipo==='sent'?'self-end':'self-start');const avatarImg=document.createElement('img');avatarImg.src=contactos.find(c=>c.id===msg.from).avatar;avatarImg.className='w-6 h-6 rounded-full flex-shrink-0';const texto=document.createElement('span');texto.textContent=msg.contenido;if(tipo==='sent'){div.appendChild(texto);div.appendChild(avatarImg);}else{div.appendChild(avatarImg);div.appendChild(texto);}chatWindow.appendChild(div);chatWindow.scrollTop=chatWindow.scrollHeight;}
...
document.addEventListener('keydown',e=>{if(e.ctrlKey&&e.altKey&&e.key.toLowerCase()==='g'){toggleConfigAndUser();}});
